...
module.exports = function() {
    ...
    //it from the coordsArray.
        ...
        direction = possibleDirections[Math.floor(Math.random()*possibleDirections.length)];
          console.log('Newish coords hash at x is '+this.coords.hash[0]+' with randomNode.y as '+randomNode.y+' and x as '+randomNode.x);      
        if (!this.coords.grid[direction.x]) {
            this.coords.hash.push(direction.x);
            this.coords.grid[direction.x] = [];
        }
        this.coords.hash[direction.x].push(direction.y);

        this.coords.hash[direction.x][direction.y] = direction;
        
        this.coords.grid[direction.x][direction.y] = this.coords.hash[direction.x][direction.y];
